...
Value format (approved by AJ on 16 September 2026): the AWSConfigINI writer writes values bare. Its package decision D15 had it double-quote every value, and the AWS CLI reads a quoted value with the quotes included, so an export would have broken `region` and `sso_session` for the CLI in every section of the file. The AWS SDKs and Tools Reference defines an entry as `setting-name=value` with no quoting (https://docs.aws.amazon.com/sdkref/latest/guide/file-format.html), `aws configure set` writes bare values, and go-ini writes bare values by default and quotes only as an escape. The reader still strips a matching quote pair, so files written by the old writer keep parsing. Values containing a double quote or a newline stay unrepresentable. Found on 15 September 2026 by testing the writer's output against AJ's AWS CLI before the first live export. The first live export on 16 September 2026 then dropped two comment blocks from the file, because the parser discarded comments that a blank line separated from the next section or key once a section had been seen. Approved by AJ the same day: such comments now attach to the next section or key and survive a write; the writer places them directly above it, so the blank line between them is not kept. Comments after the last key of the file are still dropped; AJ chose not to add a trailing-comments field for that case.
